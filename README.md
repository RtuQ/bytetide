...
面向嵌入式开发的串口与网络数据工作台。把实时日志、协议解析、波形、告警、自动回复、现场捕获和多会话对比收进一个桌面应用——从“设备有没有回包”一路追到“哪一帧、哪个字段出了问题”。

基于 Tauri 2 + Vue 3 + Rust，核心采集与规则在 Rust 侧持续运行；界面退到后台时，录制、自动回复、告警和触发式捕获仍不依赖渲染线程。数据默认留在本机，不依赖云服务。原名 Serial Tool。

> 串口 · TCP Client / Server · UDP · ASCII / HEX · 协议解析 · 实时绘图 · 自动化 · CLI · AI REST Bridge

## 预览

<p align="center">
  <img src="docs/preview-dark.png" alt="ByteTide 深色主题分析工作台" width="100%" />
</p>

<p align="center"><sub>高密度分析工作台，支持浅色与 OLED 深色主题。</sub></p>

### 工作台一览

![ByteTide 日志、图表、规则侧栏与发送区](docs/preview-workbench.png)

- **顶部**：连接标签、日志载入、日志 / 分屏 / 图表 / 对比四种中心视图
- **中心**：虚拟滚动日志与多通道波形，可拖动调整比例
- **右侧**：查找、规则、协议解析、数据绘图、现场捕获与预设库
- **底部**：解码 / 告警 / 监控 Dock，以及单发、快捷帧、序列、校验四类发送工具
...
- 自研虚拟滚动，流式接收时文本选择不跳动；视图缓冲默认 **200,000 行**，可配置 10,000–1,000,000 行，并支持向上翻页回补 Rust ring 中的旧行
...
- ASCII / HEX 双模式单发、定时发送与发送历史，`Ctrl+Enter` 快速发送
- **快捷帧库**：常用指令命名保存，一键发送
- **发送序列**：混排发送、延时、DTR / RTS 信号步骤，支持循环执行，适合上电与 Bootloader 流程
- **实时校验**：七种算法并排计算，可追加到输入框或直接存为快捷帧
...
- **现场捕获**：关键词、告警或断连触发“行车记录仪”，自动保存触发前后窗口；连续事件会合并为同一档案
- **场景自动化**：版本化 JSON 场景（发送 / 延时 / 信号线 / 等待捕获 / 断言 / 循环）驱动设备并校验响应，桌面工作台与 CLI（`bytetide run`）同一引擎、等效报告，JSON / JUnit 报告可直接接 CI；详见 [docs/automation-scenarios.md](./docs/automation-scenarios.md)
- **时序回放**：离线日志按原始时间差重放为伪实时会话，0.1–100x 变速、循环、跳转；解析 / 绘图 / 搜索 / 告警 / 对比全链路可用，发送 / 录制等写入面安全禁用；详见 [docs/replay.md](./docs/replay.md)

### 协议解析

- 导入 `bytetide.parser v1` 脚本，以声明式字段或沙箱 JavaScript 解码 RX / TX 帧
- 按会话与方向独立切帧，字段结果进入底部「解码」Dock；导入时可对最近日志回溯试运行
- 脚本超时、持续高错误率时自动终止并熔断，避免影响采集主链路
