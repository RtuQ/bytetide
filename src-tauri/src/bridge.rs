...
///
/// 运行态可见：启动/替换均**同步 bind**——命令返回前即可知道成败，
/// 绑定失败写 `runtime`（Error + last_error）供 UI 展示，不再仅 eprintln。
pub struct BridgeController {
    cfg: Arc<RwLock<BridgeConfig>>,
    runtime: Arc<RwLock<BridgeRuntime>>,
    task: Mutex<Option<async_runtime::JoinHandle<()>>>,
    /// 会话访问面：生产 = `TauriService`；测试注入 no-op——控制器可脱离 AppHandle 直测。
    service: Arc<dyn BridgeService>,
...
impl BridgeController {
    pub fn init(app: AppHandle) -> Self {
        ...
        let ctrl = Self {
            ...
            task: Mutex::new(None),
            service: Arc::new(TauriService { app }),
            config_dir: dir,
    ...
    #[cfg(test)]
    fn test_config_dir(&self) -> std::path::PathBuf {
...
mod tests {
    ...
    impl BridgeService for NoopService {
        ...
        fn bridge_follow(&self, _id: &str, _since: u64) -> Option<(Vec<BridgeLine>, u64)> {
            None
        }
        fn bridge_last_no(&self, _id: &str) -> Option<u64> {
            None
