...
const layerText = computed(() =>
  ui.hasParse ? t('parser.layerWorker') : t('parser.layerDeclarative', { n: ui.fieldsCount ?? 0 }),
)
...
// code→词条映射：值存 MessageKey，使用点 t(VERDICT_KEY[verdict]) 求值（切语言即时刷新）
const VERDICT_KEY: Record<TrialVerdict, MessageKey> = {
  ok: 'parser.verdictOk',
  suspect: 'parser.verdictSuspect',
  'no-data': 'parser.verdictNoData',
}
...
<template>
  <details class="panel" :open="open" @toggle="emit('toggle', $event)">
    <summary class="panel-head">
      ...
      <span class="panel-title">{{ t('parser.title') }}</span>
    ...
    >
      ...
      <div v-else-if="showSuspectHint" class="parser-banner warn">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 20h16a2 2 0 0 0 1.73-2Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>
        <span>{{ t('parser.suspectHint') }}</span>
      </div>
      ...
      <template v-else>
        <div class="parser-card">
          <div class="parser-card-head">
            ...
            <button class="btn btn-ghost btn-sm btn-icon" :title="t('parser.viewScript')" :aria-label="t('parser.viewScript')" @click="openViewer">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z" /><circle cx="12" cy="12" r="3" /></svg>
            </button>
            <button class="btn btn-ghost btn-sm btn-icon" :title="t('parser.reloadScript')" :aria-label="t('parser.reloadScript')" @click="reloadScript()">
            ...
            <button class="btn btn-ghost btn-sm btn-icon" :title="t('parser.unloadScript')" :aria-label="t('parser.unloadScript')" @click="unloadScript()">
          ...
          <div class="parser-switch-row">
            ...
            <span>{{ ui.enabled ? t('parser.enabledOn') : t('parser.enabledOff') }}</span>
          </div>
        </div>
        ...
        <div class="parser-stats">
          <div class="parser-stat">
            <b>{{ ui.stats.frames }}</b>
            <span>{{ t('parser.statFrames') }}</span>
          </div>
          <div class="parser-stat">
            <b>{{ rateText }}</b>
            <span>{{ t('parser.statRate') }}</span>
          </div>
          <div class="parser-stat">
            <b>{{ ui.stats.types }}</b>
            <span>{{ t('parser.statTypes') }}</span>
          </div>
        </div>
      </template>
    ...
    <div v-if="viewerOpen" class="parser-mask" @click.self="viewerOpen = false">
      <div class="parser-modal" role="dialog" :aria-label="t('parser.modalAria')">
        <div class="parser-modal-head">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /><path d="M10 13H8" /><path d="M16 13h-2" /><path d="M10 17h6" /></svg>
          <span>{{ t('parser.modalTitle', { name: ui.script?.name ?? '', version: ui.script?.version ?? '' }) }}</span>
          <span class="parser-spacer"></span>
          <button class="btn btn-ghost btn-sm btn-icon" :title="t('app.common.close')" :aria-label="t('app.common.close')" @click="viewerOpen = false">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
          </button>
        </div>
        <div class="parser-modal-body">
          <div class="parser-pane-label">{{ t('parser.sourceLabel') }}</div>
          ...
          <div v-if="ui.trialReport" class="parser-dry">
            <div class="parser-dry-head">
              <span class="parser-verdict" :class="ui.trialReport.verdict">
                {{ t(VERDICT_KEY[ui.trialReport.verdict]) }}
              </span>
              <span class="parser-dry-sum">
                {{ t('parser.drySummary', { lines: ui.trialReport.lines, frames: ui.trialReport.frames, crc: ui.trialReport.crcFailed, errors: ui.trialReport.parseErrors }) }}
              </span>
            </div>
