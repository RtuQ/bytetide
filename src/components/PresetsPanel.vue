...
<template>
  ...
  <div v-if="store.presets.length" class="preset-list">
    <div v-for="p in store.presets" :key="p.id" class="preset-row">
      ...
      <button class="btn btn-sm btn-ghost" type="button" title="应用到新建连接表单" @click="apply(p.config)">
  ...
  <div v-else class="panel-hint">暂无预设。在「新建连接」中配好参数后，可在这里保存。</div>

  <div class="sm-foot">
    <span class="panel-hint">应用回填到「新建连接」表单，需再点「连接」</span>
  </div>
</template>
