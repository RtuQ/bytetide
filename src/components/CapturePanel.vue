...
const disabledHint = computed(() =>
  active.value ? t('cap.offlineOnly') : t('cap.noSession'),
)
...
<template>
  <details class="panel">
    <summary class="panel-head">
      ...
      <span class="panel-title">{{ t('cap.title') }}</span>
      <span v-if="armed" class="badge badge-hot" :title="t('cap.armedTitle')"><span class="dot-ic"></span></span>
    ...
    <div class="panel-body cap-body">
      <template v-if="editable && active">
        <div class="cap-line">
          <label class="check">
            ...
            <span>{{ t('cap.enable') }}</span>
          </label>
          <span class="send-spacer"></span>
          <label class="check" :title="t('cap.onDisconnectTitle')">
            <input
              type="checkbox"
            ...
            <span>{{ t('cap.onDisconnect') }}</span>
          </label>
        </div>

        <div class="cap-line">
          <span class="cap-lbl">{{ t('cap.pre') }}</span>
          <input class="input input-mono cap-sec" type="number" v-model.number="preSec" min="10" max="1800" step="10" :aria-label="t('cap.preAria')" @change="setPre" />
          <span class="cap-unit">s</span>
          <span class="send-spacer"></span>
          <span class="cap-lbl">{{ t('cap.post') }}</span>
          <input class="input input-mono cap-sec" type="number" v-model.number="postSec" min="10" max="1800" step="10" :aria-label="t('cap.postAria')" @change="setPost" />
          <span class="cap-unit">s</span>
        </div>
        <div class="cap-hint">{{ t('cap.hint') }}</div>

        <div class="cap-sub">{{ t('cap.rulesSub') }}</div>
        <div v-for="r in active.capture.rules" :key="r.id" class="cap-rule">
          <input
            class="input input-mono cap-pattern"
            :value="r.pattern"
            :placeholder="t('cap.patternPh')"
            @change="updRule(r.id, { pattern: ($event.target as HTMLInputElement).value })"
          />
          <select
            class="select cap-mode"
            :value="r.useRegex ? 're' : 'text'"
            :aria-label="t('cap.modeAria')"
            @change="updRule(r.id, { useRegex: ($event.target as HTMLSelectElement).value === 're' })"
          >
            <option value="text">{{ t('cap.modeText') }}</option>
            <option value="re">{{ t('cap.modeRegex') }}</option>
          </select>
          <button class="btn btn-ghost btn-sm" :title="t('cap.delRule', { name: r.pattern || r.id })" :aria-label="t('cap.delRule', { name: r.pattern || r.id })" @click="delRule(r.id)">
        ...
        <button class="btn btn-ghost btn-sm cap-add" @click="addRule">{{ t('cap.addRule') }}</button>
      </template>
      <div v-else class="cap-hint">{{ disabledHint }}</div>

      <div class="cap-sub">{{ t('cap.archivesSub') }}</div>
      <div v-if="store.captures.length" class="cap-list">
        <div v-for="f in store.captures" :key="f.path" class="cap-item" :title="f.path">
          ...
          <span class="cap-acts">
            <button class="btn btn-sm" :title="t('cap.openTitle')" @click="openCapture(f.path)">{{ t('cap.open') }}</button>
            <button class="btn btn-ghost btn-sm" :title="t('cap.delArchiveTitle')" :aria-label="t('cap.delArchive', { name: f.fileName })" @click="store.deleteCapture(f.path)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m3 0v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg>
            </button>
          </span>
        </div>
      </div>
      <div v-else class="cap-hint">{{ t('cap.emptyArchives') }}</div>
      <div class="cap-line">
        <button class="btn btn-sm" :title="t('cap.refreshTitle')" @click="store.loadCaptures()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-2.64-6.36"/><path d="M21 3v6h-6"/></svg>
          <span>{{ t('cap.refresh') }}</span>
        </button>
        <button class="btn btn-sm" :title="t('cap.openDirTitle')" @click="capDir && openPath(capDir).catch(() => {})">
