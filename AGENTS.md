...
.app (column, 100vh)
 ...
 └─ .app-main (row, flex:1)
     ...
     │   ├─ .center-body
     │   │   ├─ LogView    ← 工具栏 + 虚拟滚动日志(横向可滚) + 空状态；行右键菜单=
     │   │   │                  重发此帧(lineBytes 原始字节→HEX 直发，offline/未连接禁用)/
     │   │   │                  复制文本/复制 HEX（teleport 到 body，backdrop 点击/Esc 关闭）；
     │   │   │                  复制文本选区感知：openCtx 瞬间快照划选（点击菜单项时选区已清），
     │   │   │                  按消息列逐行裁剪以 \n 拼接（composables/selectionText，边界
     │   │   │                  点含祖先-后代 cdp 细化），无选区/复制 HEX 恒为整行；
     ...
     │   │              （serialtool.dock）
     │   └─ SendPanel  ← 发送区（<details> 折叠条，默认收起）四页签：单发(原样保留，含
     │                  定时发送)/快捷帧(点击即发，「存当前文本」入库，管理模式改名删除)/
     │                  序列(发送+延时+DTR·RTS 信号三类步骤，可循环+轮间隔；useSequenceRunner
     │                  语义在 store.runSequence——全局同时仅一个序列，断开/停止即中止)/
     │                  校验(七算法并排实时算，「＋追加」拼回单发框、「存为快捷帧」)；
     │                  页签行右侧 DTR/RTS 拨杆（仅串口源已连接可用，切会话/断开重置显示）
     ...
     └─ .app-sidebar   ← 分组粘头（.group-head sticky）四组，overflow-y:auto：
         ...
         数据(PlotConfigPanel/CapturePanel=现场捕获：触发规则+断连捕获+前后窗口+
         档案列表[打开=离线会话/删除/刷新/打开目录]) · 库(ConfigPresetsPanel/AiNotesPanel)
...
- 会话级 UI 偏好字段（showLineNo/showDir/droppedLines/bookmarks/aiNotes/alerts/filters/**capture**…）**必须同时**改三处：`makeSession` 默认值、`reconnectSession` 的 carried 迁移清单、相关 actions。clearLog 会重置 lineCounter 与 droppedLines 并清空 bookmarks 与 aiNotes（aiNotes 同时回写后端镜像清空；droppedLines 在重连迁移中保留）。`ringDropped`（ring 覆盖缺口，StatusBar「Ring 丢 N」）与 `evictedPending`（视口锚定待补偿，`takeEvicted(id)` 取走即清零）同走三处纪律，但两者**重连不迁移、清屏归零**（新 ring/新视图从零计）。`capture`（现场捕获配置）经 `updateCapture` 合并后随 `pushLiveRules` 整包推送（与规则同类：配置非数据，清屏不清）。
- localStorage 键：`serialtool.theme/.locale(界面语言,zh-CN|en)/.lastPortConfig/.logConfig/.searchHistory/.portPresets/.configPresets/.alertSound/.update.lastCheck/.update.dismissedVersion/.sidebar(宽+收起)/.panels(侧栏面板开合)/.centerSplit(日志↔图表高度比)/.dock(底部dock高度/收起/页签)/.sendPresets(快捷帧,全局,cap 50)/.sendSequences(发送序列,全局)/.notifications(通知总开关)`。预设库 payload 各类别形状校验在 `applyConfigPreset/importConfigPresets`；快捷帧/序列形状校验在 `loadSendPresets/loadSendSequences`（`isSeqStep` 逐步骤验证）。
- **界面多语言（feat/i18n）**：`src/i18n/` 自研轻量方案——`t(key, params?)` 查表 + `{name}` 插值，`locale` 模块级单例 ref（`import` 时同步初始化：持久化值 > 系统语言 `detectSystemLocale`，zh*/en* 首个命中）；t() 内读 `locale.value`，模板/computed 调用点自动随切语言刷新。**纪律：模块级映射表禁存文案值**——动态映射存 `MessageKey`（`Record<Code, MessageKey>`），使用点 `t(MAP[code])`；不要在 setup 顶层用 const 接 t() 结果。词典按 9 域拆 `messages/{zh-CN,en}/{app,logview,send,rule,lib,scen,dock,logic,errors}.ts` **成对文件**，zh 是键的唯一事实来源（`MessageKey` = 全域并集），en 用 `Record<keyof typeof <域>Zh, string>` 编译期强制 + `i18n.test.ts` 键集互检兜底；**新增词条只写自己域的一对文件，不动 `*/index.ts`**；单文件 ≤1200 行（架构检查），塞不下就拆新域。vitest 全局 setup（`src/test/vitest-setup.ts`）锁 zh-CN——jsdom `navigator.language=en-US` 会让跟随系统判定污染中文断言。index.html 内联脚本同步 `<html lang>`（与 `useLocale` 两处默认，AGENTS 纪律）；窗口标题 zh `ByteTide · 字节潮` / en `ByteTide`。**后端错误码机制**：Rust 错误一律 `"{code}|{detail}"`（core `err_msg`/src-tauri `cmd_err`，detail=io 文本不翻译），码全集契约 `src/ipc/errorCodes.ts`（**改 Rust 错误必同步此清单与 `errors.<code>` 词典**，`errorCodes.test.ts` 守卫词典↔清单双向一致）；前端展示走 `errorMessage()`，`useToast.connectionErrorHint` 按 code 分派（open_* 类再看 detail 英文关键词细分；generic 保留旧文本关键词兜底向后兼容）。本期范围外（保持中文）：`parser/engine.ts` 校验串（worker 侧）、`builtinExample.ts` 示例数据、CLI 输出。
...
- **数据流 = 拉模型（feat/pull-based-view 起）**：后端 ring 是唯一真相（`no` 游标单调递增、清屏不回退）；前端 `useTauriEvents` 按会话自适应节奏（`pullPacing.ts` 纯控制律 + 单链 setTimeout 调度器：拉到数据立即贴 **25ms 地板**，连续空拍超过 3 拍宽限后 ×1.8 乘性爬回 **200ms 天花板**——天花板不高于旧固定值，静默期首行延迟零回归）按 `pullNo` 调 `ring_lines_no_cmd` 拉 delta（`appendPulled` 入表），**不再有 `log` 事件流**（40ms 推事件曾把 WebView2 渲染进程调度饿死成死亡螺旋，实测积压 15 分钟、1s 定时器饿到 48s 才醒）。拉的方向自带背压：IPC 频率自限于渲染进程真实处理能力，勿回加高频 emit。渲染进程被节流时最坏滞后=一个拉取周期（25–200ms），醒来一次拉齐即收敛，被节流期 setTimeout 自动晚触发即被动降频。新增实时数据通道时走游标拉取，勿回加高频 emit。CLI（bytetide-cli）是 ring 的第二个消费者：每 50ms 调 `ring_lines_after_no` 游标拉取、零事件流，新增消费者照此办理。`RING_CAP` = 100000（manager.rs，≈17MB/会话）；前端 `PULL_MAX_PAGES` = 24（24×5000=12 万 ≥ RING_CAP，一轮必收敛）——**调 RING_CAP 必须同步复核 PULL_MAX_PAGES**；`appendPulled` 在去重过滤之前按游标缺口累计 `ringDropped`（首行 ringNo > pullNo+1 即有行被 ring 覆盖）。
...
- **触发式现场捕获（行车记录仪）= 读线程评估**：会话级 `capture` 配置随 `pushLiveRules` 第四参推送（serde camelCase+default 向后兼容）。`stream_loop` 内三触发：①`capture_eval` 关键词/正则命中 ②告警命中联动（fired 非空即取其 pattern）③`on_disconnect` 断连/读错误回溯。命中=`capture_start`：`next_capture_path`（`{id}-cap-YYYYMMDD-HHMMSS[-N].log`，同秒 -2/-3）在 `sessions/captures/` 建档，写 `#` 头注释行（前端 `parseLogFile` 跳 `#`），`RingBuf::lines_since_epoch`（epoch_millis 二分；缺失标记=发生过淘汰(seq>len)且现存首行晚于窗口起点——会话初数据天然不足不算）回溯 pre 窗口（`clamp_capture_window` 1s–30min）；armed 期间每行（RX/TX 回显/空闲刷出）经 `cap_on_line` 续写，post 窗口到期 `cap_finalize`；armed 中再命中**顺延 deadline**（连续事故合并一档）。事件对：`capture-active`（armed 置位，前端 StatusBar「捕获中」红点呼吸+面板图标变色）→`capture_saved`→`capture-saved`（解除+刷新档案列表）。落成走 `EventSink::capture_saved`。字节落盘与录制同为 lossy TSV（原始字节本就不落盘）。档案命令：`list_captures_cmd`/`delete_capture_cmd`（守卫：仅 captures 目录内 .log）/`captures_dir_cmd`。
- **发送区升级与信号线**：DTR/RTS 走 `PortCmd::Signal{pin,level}` → `Link`（stream_loop 从泛型 `T: Read+Write` 改为具体 `Link` 枚举=Serial/Net 复合视图——io 读写与引脚置位都唯一借用同一 `Box<dyn SerialPort>`，无法拆双 `&mut`；网络源报「无信号线」）。命令 `set_signal_cmd`；serialport 引脚方法返回其自有 `Result<T, serialport::Error>` 别名需 map 成 io::Error。快捷帧/序列为全局库（非会话级）；序列运行态 `store.seqRun`（响应式进度）+模块级停止旗标，同一时刻仅一个序列、断开/失败即中止。
- **通知系统 v2（右上卡片流，通知重设计）**：`ToastHost` = `TransitionGroup` 右上堆叠卡片（B 的位置 + A 的卡片语言：左缘 4px 语义色条 `--kind` + 图标徽标 + 底部倒计时条；进/离场 240/160ms、`.toast-move` FLIP 补位，leave-active 脱流锚右上）。**关闭由 `useToast` 的 setTimeout 驱动**（勿改回 animationend 驱动——窗口被遮挡冻结时 CSS 动画事件不可靠，demo 实测复现）；悬停暂停 = 计时器记账（`pauseToast`/`resumeToast`）+ WAAPI 暂停进度条，同进退。断开提示 = warning 6s + tag `'disconnect'`，连接成功时 `dismissByTag` 收掉未过期的断开提示。**热插拔通知**：后端 `hotplug.rs` 轮询 emit 的 `port-changed` 只有完整端口列表不带方向，前端 `usePortNotifications.consumePortDiff` 对前后列表求差（**首帧只建基线不弹**——启动时已插着的端口不刷「已接入」；`diffPorts`/`describePort` 为纯函数有单测）。开关在 SettingsPopover「通知」分组单个 `.switch` 总开关（`useNotificationPrefs`，`serialtool.notifications`，默认开，统一门控连接/断开与串口插拔提示）。样式 demo 存档 `docs/notification-demo.html`（自包含，可删）。
- **事件出口 = `EventSink` trait（core 去 tauri 化）**：core 不再 import tauri；状态/错误/告警/捕获（`status`/`error`/`alert_hits`/`capture_saved`/`capture_active`）经 `sink.rs` 的 `EventSink` trait 送出，桌面端 `src-tauri/src/gui_sink.rs` 转发为原事件名（`session-status`/`session-error`/`alert-hit`/`capture-saved`/`capture-active`）。改事件名必须同步前端 `useTauriEvents`。`PortManager::connect` 现签名 `(config, log_config, sink: Arc<dyn EventSink>, sessions_dir)`，`sessions_dir` 传空 = 不落盘（CLI 默认，现场捕获一并停用）。
