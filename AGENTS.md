...
- **落盘录制开关/分段（recOn）**：会话级 `recOn`（默认 true，重连迁移，三处同步照旧；关=仅停写文件，ring/视图不受影响）。工具栏「录制」→ `set_recording_cmd`（关=`PortCmd::RecOff` flush+关文件；开=另起新分段）；「分段」→ `rotate_log_cmd` 始终另起 `{基准名}-YYYYMMDD-HHMMSS.log`（基准名=连接时解析路径，存 `SessionHandle.log_base` 防 stem 越叠越长；同秒冲突 `-2/-3` 递增；`log_path` 随最新分段更新，「打开日志」指向当前文件）。两命令经 `PortCmd::RecOn/RecOff` 在读线程内切 writer；重连后原会话若暂停会补发暂停。
